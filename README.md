...
| `secrets` | Two separate AWS Secrets Manager secrets — `{app}/identity/{environment}/jwt-private-key` and `{app}/identity/{environment}/hmac-key`, each a plain-string value (not a JSON blob), encrypted with the KMS key from `module.kms`; the actual values are injected at deploy time, not by Terraform |
