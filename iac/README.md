...
| `secrets` | Two AWS Secrets Manager secrets, one per credential (not a combined JSON blob): `{app_name}/identity/{environment}/jwt-private-key` and `{app_name}/identity/{environment}/hmac-key` (e.g. `rentifyx/identity/production/jwt-private-key`), each encrypted with the KMS key from `module.kms` and holding a plain-string value that `SecretsManagerConfigurationProvider` fetches directly — matches `rentifyx-communications-api`'s per-credential pattern, lets each value be rotated independently and inspected via `aws secretsmanager get-secret-value` with no JSON parsing. Terraform only creates both secrets with placeholder values (`REPLACE_AT_DEPLOY_TIME`) and then `ignore_changes` on `secret_string` — the real values are injected out-of-band at deploy time, never by Terraform/committed to state in the clear. |
